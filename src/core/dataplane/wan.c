...
{
    if (!pkt || !len || *len < 14 || *len > capacity)
        return -EINVAL;
    const uint16_t wire_ethertype = ((uint16_t)pkt[12] << 8) | pkt[13];

    if (wire_ethertype == NE_L2_TCP_ETHERTYPE)
        return core_tcp_handle_wan_lan(cfg, pkt, len, capacity);
    if (wire_ethertype == NE_L2_UDP_ETHERTYPE)
        return core_udp_handle_wan_lan(cfg, pkt, len, capacity);
    if (wire_ethertype == NE_L2_PING_ETHERTYPE)
        return core_ping_handle_wan_lan(cfg, pkt, len, capacity);
    if (wire_ethertype == NE_L2_OSPF_ETHERTYPE)
        return core_ospf_handle_wan_lan(cfg, pkt, len, capacity);
