...
static int core_ping_fragment(uint8_t *pkt, uint32_t *len, uint32_t capacity,
                            uint8_t policy_id, uint8_t core_id,
                            uint8_t *second, uint32_t *second_len,
                            const uint8_t key[32]);
static int core_ping_reassemble(uint8_t *pkt, uint32_t *len,
                              uint32_t capacity, uint8_t policy_id,
                              uint8_t core_id);

static int core_ping_decrypt(uint8_t *pkt, uint32_t *len, const uint8_t key[32]);
static int core_ping_encrypt(uint8_t *pkt, uint32_t *len, uint32_t capacity,
                             uint8_t policy_id, uint8_t core_id,
                             const uint8_t key[32]);
                            ...
                            uint8_t *wan_idx)
{
    ...
    if (!pkt || !len || !second_len || *len > capacity)
        return -EINVAL;
    *second_len = 0;
    rc = core_ping_per_flow(cfg, pkt, *len, &selected);
    if (rc != 0)
        return rc;
    ...
    if (rc != 0)
        return rc;
    if (*len + 30u > PATH_MTU)
        rc = core_ping_fragment(pkt, len, capacity, policy_id, core_id,
                             second, second_len, key);
    else
        rc = core_ping_encrypt(pkt, len, capacity, policy_id, core_id, key);
                            ...
                            uint32_t *len, uint32_t capacity)
{
    ...
    if (rc != 0)
        return rc;
    rc = core_ping_decrypt(pkt, len, key);
    memset(key, 0, sizeof(key));
    if (rc != 0)
        return rc;
    rc = core_ping_reassemble(pkt, len, capacity, policy_id, core_id);
    if (rc != 0)
        return rc;
...
static uint32_t core_ping_get32(const uint8_t *p)
{
    return ((uint32_t)p[0] << 24) | ((uint32_t)p[1] << 16) |
           ((uint32_t)p[2] << 8) | p[3];
}

static void core_ping_put32(uint8_t *p, uint32_t value)
{
...
static void core_ping_write_frag(uint8_t *p, uint8_t kind,
                               uint32_t epoch, uint32_t seq)
{
    ...
    core_ping_put32(p + 5, epoch);
    core_ping_put32(p + 9, seq);
    core_ping_put32(p + 13, seq);
}

static uint64_t core_ping_now_ns(void)
{
    struct timespec ts;
                              ...
                              uint8_t core_id)
{
    ...
    if (*len >= 34 && (pkt[14] >> 4) == 4)
        return pkt[23] == 1 ? 0 : -EBADMSG;
    if (*len < 32 || pkt[14] != 0x5b || pkt[15] != 'I' ||
        pkt[16] != 'C' || pkt[17] != 1 ||
        (pkt[18] >> 4) != 1 || (kind = pkt[18] & 15u) > 1)
        return -EBADMSG;
    epoch = core_ping_get32(pkt + 19);
    datagram = core_ping_get32(pkt + 27);
    if (core_ping_get32(pkt + 23) != datagram || !datagram)
        return -EBADMSG;
    id = ((uint64_t)epoch << 32) | datagram;
    now = core_ping_now_ns();
...
static int core_ping_encrypt(uint8_t *pkt, uint32_t *len, uint32_t capacity,
                             ...
                             const uint8_t key[32])
{
    return core_l2_pqc_encrypt(pkt, len, capacity, NE_L2_PING_ETHERTYPE,
                               policy_id, core_id, key);
}

static int core_ping_decrypt(uint8_t *pkt, uint32_t *len, const uint8_t key[32])
{
    return core_l2_pqc_decrypt(pkt, len, *len, NE_L2_PING_ETHERTYPE, key);
}
                       ...
                       uint32_t len, uint8_t *wan_idx)
{
    ...
    if (!cfg || !pkt || !wan_idx || len < 34 || pkt[12] != 8 || pkt[13] != 0 ||
        (pkt[14] >> 4) != 4 || pkt[23] != IPPROTO_ICMP_VAL)
        return -EINVAL;
    ihl = (uint32_t)(pkt[14] & 15u) * 4u;
    if (ihl < 20 || len < 14u + ihl + 8u)
        return -EINVAL;
    for (uint32_t i = 26; i < 34; i++)
        hash = (hash ^ pkt[i]) * 16777619u;
    if (pkt[14 + ihl] == 0 || pkt[14 + ihl] == 8) {
        hash = (hash ^ pkt[18 + ihl]) * 16777619u;
        hash = (hash ^ pkt[19 + ihl]) * 16777619u;
    }
