    ...
    uint8_t core_id, struct core_packet_batch *out)
{
    ...
    if (rc) return rc;
    rc = core_l2_pqc_fragment(pkt, len, capacity, NE_L2_OSPF_ETHERTYPE,
    ...
    uint32_t *len, uint32_t capacity)
{
    ...
    int rc = core_l2_pqc_reassemble(pkt, len, capacity, NE_L2_OSPF_ETHERTYPE);
    if (rc) return rc;
    uint8_t policy_id = pkt[*len - 2];
    rc = core_key_get(policy_id, key, sizeof(key));
    if (rc) return rc;
    rc = core_l2_pqc_decrypt(pkt, len, capacity, NE_L2_OSPF_ETHERTYPE, key);
    memset(key, 0, sizeof(key));
    if (rc) return rc;
